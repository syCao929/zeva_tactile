...
class OmniMoTModel(ImaginaireModel):
    ...
    def build_net(self, dtype: torch.dtype, *, lora_enabled: bool | None = None) -> torch.nn.Module:
        ...
        with torch.device("meta"):
            ...
            network_config = Cosmos3VFMNetworkConfig(
                ...
                # At runtime TOML/Hydra materializes this attrs default as an
                # OmegaConf DictConfig, so use the mapping protocol rather
                # than attrs.asdict.
                behavior_stage2_config=dict(self.config.behavior_stage2),
                proprio_condition_config=dict(self.config.proprio_condition),
                residual_expert_config=dict(self.config.residual_expert),
            )
            ...
            if bool(residual_cfg.get("enabled", False)):
                if lora_enabled:
                    raise ValueError("Residual expert and LoRA cannot be enabled together")
                trainable = 0
                for name, parameter in net.named_parameters():
                    keep = name.startswith("action_residual_head.")
                    parameter.requires_grad_(keep)
                    if keep:
                        trainable += parameter.numel()
                if trainable == 0:
                    raise RuntimeError("Residual expert is enabled but action_residual_head is absent")
                log.info(
                    f"Frozen-base residual expert: trainable_params={trainable:,}; "
                    "all Cosmos-Nano-DROID base parameters require_grad=False"
                )
    ...
    def _pack_input_sequence(
        ...
        return pack_input_sequence(
            ...
            num_behavior_prefix_tokens=(
                self.config.behavior_stage2.global_prefix_tokens
                if self.config.behavior_stage2.enabled
                else 0
            ),
            num_online_memory_prefix_tokens=(
                self.config.behavior_stage2.online_prefix_tokens
                if self.config.behavior_stage2.enabled and self.config.behavior_stage2.online_memory_enabled
                else 0
            ),
            num_proprio_prefix_tokens=(
                self.config.proprio_condition.prefix_tokens
                if self.config.proprio_condition.enabled
                else 0
            ),
        )

    def _attach_proprio_condition(
        self,
        packed_sequence: PackedSequence,
        gen_data_clean: GenerationDataClean,
    ) -> None:
        """Attach the dense per-action-sample robot state after packing."""
        cfg = self.config.proprio_condition
        if not cfg.enabled:
            return
        values = gen_data_clean.proprio_condition
        if values is None:
            raise KeyError("proprio_condition.enabled requires dataset key 'proprio'")
        tensors = []
        for value in values:
            tensor = value
            while tensor.ndim > 1 and tensor.shape[0] == 1:
                tensor = tensor[0]
            if tensor.ndim != 1 or tensor.shape[0] != cfg.input_dim:
                raise ValueError(f"Expected proprio [{cfg.input_dim}], got {tuple(tensor.shape)}")
            tensors.append(tensor)
        packed_sequence.proprio_condition = torch.stack(tensors).to(
            device=packed_sequence.text_ids.device, dtype=self.precision
        )

    def _attach_stage2_behavior(
        self,
        packed_sequence: PackedSequence,
        data_batch: dict,
        gen_data_clean: GenerationDataClean,
        ...
        samples, hence the explicit reindexing by ``action_sample_indices``.
        """
        cfg = self.config.behavior_stage2
        if not cfg.enabled:
            return
        required = ("behavior_global", "behavior_phase", "behavior_effect", "behavior_effect_valid")
        if any(name not in data_batch for name in required):
            raise KeyError("Stage-2 dataset must emit behavior_global, behavior_phase, behavior_effect and behavior_effect_valid")

        def as_batch(value: object, name: str) -> torch.Tensor:
            if isinstance(value, torch.Tensor):
                tensor = value
            elif isinstance(value, (list, tuple)) and all(isinstance(item, torch.Tensor) for item in value):
                tensor = torch.stack(list(value))
            else:
                raise TypeError(f"Unsupported {name} batch type: {type(value)!r}")
            # PackingDataLoader keeps a one-element inner list per sample for
            # non-sequence auxiliary tensors, producing [B,1,D].
            if tensor.ndim == 3 and tensor.shape[1] == 1:
                tensor = tensor[:, 0]
            if tensor.ndim != 2:
                raise ValueError(f"Expected {name} [B,D], got {tuple(tensor.shape)}")
            return tensor

        global_features = as_batch(data_batch["behavior_global"], "behavior_global")
        phase_features = as_batch(data_batch["behavior_phase"], "behavior_phase")
        effect_features = data_batch["behavior_effect"]
        effect_valid = data_batch["behavior_effect_valid"]
        if isinstance(effect_features, (list, tuple)):
            effect_features = torch.stack(list(effect_features))
        if isinstance(effect_valid, (list, tuple)):
            effect_valid = torch.stack(list(effect_valid))
        if not isinstance(effect_features, torch.Tensor) or not isinstance(effect_valid, torch.Tensor):
            raise TypeError("Stage-2 effect history must be tensor-valued")
        # The packing loader adds a singleton list axis to auxiliary tensors.
        if effect_features.ndim == 4 and effect_features.shape[1] == 1:
            effect_features = effect_features[:, 0]
        if effect_valid.ndim == 3 and effect_valid.shape[1] == 1:
        ...
        phase_features = phase_features.index_select(0, phase_indices).to(
            device=packed_sequence.text_ids.device, dtype=self.precision
        )
        effect_features = effect_features.index_select(0, global_indices.to(effect_features.device)).to(
            device=packed_sequence.text_ids.device, dtype=self.precision
        )
        ...
        targets = []
        for action in x0_actions:
            start = int(cfg.leading_condition_steps)
            target = action[start : start + cfg.horizon, : cfg.action_dim]
            if target.shape != (cfg.horizon, cfg.action_dim):
                raise ValueError(f"Expected policy action target [{cfg.horizon},{cfg.action_dim}], got {tuple(target.shape)}")
            targets.append(target)
        packed_sequence.behavior_global = global_features
        ...
        if cfg.online_memory_enabled:
            ...
            online_context = as_batch(data_batch["behavior_online_context"], "behavior_online_context")
            if online_context.shape[-1] != cfg.online_context_dim:
                raise ValueError(
                    f"Expected behavior_online_context [...,{cfg.online_context_dim}], got {tuple(online_context.shape)}"
                )
            packed_sequence.behavior_online_context = online_context.index_select(
                0, global_indices.to(online_context.device)
            ).to(device=packed_sequence.text_ids.device, dtype=self.precision)
        packed_sequence.behavior_prior_target = torch.stack(targets).to(device=packed_sequence.text_ids.device, dtype=self.precision)
    ...
    def training_step(
        ...
        teacher_anchor_weights: torch.Tensor | None = None
        r3_action_sample_weights: torch.Tensor | None = None
        if self.config.r3_event_weighted:
            task_key = self.config.r3_task_weight_key
            event_key = self.config.r3_event_weight_key
            task_weights_full = data_batch.get(task_key)
            if task_weights_full is None:
                task_weights_full = torch.ones(len(sequence_plans), dtype=torch.float32)
            task_weights_full = torch.as_tensor(task_weights_full, dtype=torch.float32).reshape(-1)
            event_values = data_batch.get(event_key)
            if event_values is None:
                event_values = torch.zeros(len(sequence_plans), dtype=torch.float32)
            event_values = torch.as_tensor(event_values, dtype=torch.float32).reshape(-1)
            if task_weights_full.numel() != len(sequence_plans) or event_values.numel() != len(sequence_plans):
                raise ValueError(
                    "R3 per-sample metadata must align with sequence plans: "
                    f"task={task_weights_full.numel()} event={event_values.numel()} plans={len(sequence_plans)}"
                )
            event_scale = float(self.config.r3_event_weight)
            if event_scale < 0:
                raise ValueError(f"r3_event_weight must be non-negative, got {event_scale}")
            r3_action_sample_weights = task_weights_full * (
                1.0 + event_values.clamp(0.0, 1.0) * (event_scale - 1.0)
            )
            r3_action_sample_weights = r3_action_sample_weights[action_sample_indices]
        if self.config.lora_teacher_anchor_weight > 0:
            anchor_key = self.config.lora_teacher_anchor_data_key
            if anchor_key not in data_batch:
                raise KeyError(
                    f"lora_teacher_anchor_weight is enabled but data batch has no {anchor_key!r}; "
                    "set an explicit per-sample weight on every training dataset"
                )
            anchor_weights_full = torch.as_tensor(data_batch[anchor_key], dtype=torch.float32).reshape(-1)
            if anchor_weights_full.numel() != len(sequence_plans):
                raise ValueError(
                    f"Expected {len(sequence_plans)} teacher-anchor weights, got {anchor_weights_full.numel()}"
                )
            teacher_anchor_weights = anchor_weights_full[action_sample_indices]
        ...
        self._attach_proprio_condition(packed_sequence, gen_data_clean)
        self._attach_stage2_behavior(packed_sequence, data_batch, gen_data_clean, action_sample_indices)

        # Optional frozen base-policy teacher pass. Every FSDP rank executes
        # this forward even when its local sample weights are all zero; keeping
        # collective counts identical is required for rank-partitioned data.
        teacher_preds_action: list[torch.Tensor] | None = None
        if self.config.lora_teacher_anchor_weight > 0:
            from cosmos_framework.utils.generator.lora import disable_lora_adapters

            teacher_memory = self.build_memory_state(
                packed_sequence, memory_info
            )  # pylint: disable=assignment-from-none
            with torch.no_grad(), disable_lora_adapters(self.net):
                teacher_out = self.denoise(
                    data_batch_packed=packed_sequence,
                    memory=teacher_memory,
                )
            teacher_preds_action = [pred.detach() for pred in teacher_out["preds_action"]]
            del teacher_out, teacher_memory
        ...
        loss, losses_dict = self._compute_losses(
            ...
            teacher_preds_action=teacher_preds_action,
            teacher_anchor_weights=teacher_anchor_weights,
            r3_action_sample_weights=r3_action_sample_weights,
        )
        ...
        if iteration <= 10 or iteration % 1000 == 0:
            diagnostic_names = (
                ...
                "teacher_anchor_loss_action",
                "behavior_prior_nll",
            )
            diagnostic_values = []
            for name in diagnostic_names:
                if name in losses_dict:
                    value = losses_dict[name].detach().float().mean().item()
                    diagnostic_values.append(f"{name}={value:.8f}")
            if diagnostic_values:
                log.info(
                    "stage2_loss_components: " + ", ".join(diagnostic_values) + f" (iteration {iteration})",
                    rank0_only=True,
                )
    ...
    def _compute_losses(
        ...
        if self.config.action_gen:
            if data_batch_packed.action is not None:
                ...
                fm_loss_action, _ = self._compute_flow_matching_loss(
                    ...
                    normalize_by_active=normalize_by_active,
                    sample_weights=r3_action_sample_weights,
                )
                ...
                if teacher_preds_action is not None:
                    if teacher_anchor_weights is None:
                        raise ValueError("teacher_anchor_weights are required with teacher_preds_action")
                    teacher_anchor_loss = compute_teacher_anchor_loss(
                        pred=out_net["preds_action"],
                        teacher_pred=teacher_preds_action,
                        condition_mask=data_batch_packed.action.condition_mask,
                        sample_weights=teacher_anchor_weights,
                        has_valid_tokens=has_noisy_tokens(data_batch_packed.action),
                        raw_action_dim=data_batch_packed.action.raw_action_dim,
                    )
                    total_loss += (
                        teacher_anchor_loss
                        * self.config.lora_teacher_anchor_weight
                        * rf_cfg.action_loss_weight
                    )
                    losses_dict["teacher_anchor_loss_action"] = teacher_anchor_loss
            else:
        ...
        if "behavior_prior_mean" in out_net:
            prior_loss = gaussian_prior_nll(
                data_batch_packed.behavior_prior_target,
                out_net["behavior_prior_mean"],
                out_net["behavior_prior_std"],
            )
            total_loss += prior_loss * self.config.behavior_stage2.prior_loss_weight
            losses_dict["behavior_prior_nll"] = prior_loss
    ...
    def _get_velocity(
        ...
        *,
        ...
        stage2_data_batch: dict | None = None,
        stage2_action_sample_indices: list[int] | None = None,
    ) -> list[torch.Tensor]:
        """
        ...
        gen_data_for_packing = GenerationDataClean(
            ...
            action_residual_gate=gen_data_clean.action_residual_gate if has_action else None,
            proprio_condition=gen_data_clean.proprio_condition if has_action else None,
            # Sound fields
        ...
        if packed_sequence_template is None:
            ...
            packed_sequence.to_cuda()
            self._attach_proprio_condition(packed_sequence, gen_data_for_packing)
        else:
    ...
    def generate_samples_from_batch(
        ...
        **kwargs,
        ...
        if self.config.behavior_stage2.enabled:
            stage2_action_sample_indices = [
                index for index, plan in enumerate(sequence_plans) if plan.has_action
            ]
        ...
        if reuse_pack_templates:
            ...
            self._attach_proprio_condition(cond_packed_sequence_template, gen_data_clean)
            if guidance != 1.0 or velocity_postprocess_builder is not None:
                ...
                self._attach_proprio_condition(uncond_packed_sequence_template, gen_data_clean)
            ...
            def velocity_fn(noise_x: list[torch.Tensor], timestep: torch.Tensor) -> list[torch.Tensor]:
                ...
                def _single_velocity_fn(tokens: list[list[int]], skip_text_tokens: bool) -> list[torch.Tensor]:
                    ...
                    return self._get_velocity(
                        ...
                        stage2_data_batch=data_batch,
                        stage2_action_sample_indices=stage2_action_sample_indices,
                    )
    ...
    def _slice_gen_data_clean(self, gen_data_clean: GenerationDataClean, start: int, limit: int) -> GenerationDataClean:
        ...
        if has_action:
            ...
            action_residual_gate = (
                gen_data_clean.action_residual_gate[start:limit]
                if gen_data_clean.action_residual_gate is not None
                else None
            )
        else:
        ...
        return GenerationDataClean(
            ...
            proprio_condition=(
                gen_data_clean.proprio_condition[start:limit]
                if has_action and gen_data_clean.proprio_condition is not None
                else None
            ),
    ...
    def get_data_and_condition(
        ...
        action_residual_gate = unwrap_and_densify(
            data_batch.get("residual_gate", None), self.tensor_kwargs_fp32
        )
        # Serving wraps a single sample's tensors with a leading batch
        # dimension (``[1,T,1]``), while the packed sequence expects one
        # ``[T,1]`` gate per action payload.  Training already supplies
        # ``[T,1]``.  Normalize both forms here before the gate is attached to
        # ModalityData; otherwise the flattened batch dimension can be
        # mistaken for action-token positions during inference.
        if action_residual_gate is not None:
            normalized_gates: list[torch.Tensor] = []
            for gate in action_residual_gate:
                while gate.ndim > 2 and gate.shape[0] == 1:
                    gate = gate.squeeze(0)
                if gate.ndim == 1:
                    gate = gate.unsqueeze(-1)
                if gate.ndim != 2 or gate.shape[-1] != 1:
                    raise ValueError(
                        "residual_gate entries must have shape [T,1] (or a singleton-leading "
                        f"batch dimension), got {tuple(gate.shape)}"
                    )
                normalized_gates.append(gate)
            action_residual_gate = normalized_gates
        ...
        proprio_condition = unwrap_and_densify(data_batch.get("proprio", None), self.tensor_kwargs_fp32)
        return GenerationDataClean(
