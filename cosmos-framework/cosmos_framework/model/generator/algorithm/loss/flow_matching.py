...
def compute_flow_matching_loss(
    ...
    Args:
        ...
        normalize_by_active: When True, normalize per-instance loss by the count of
            ...
            denominator.
        sample_weights: Optional per-sample weights ``[B]`` applied to both the
            plain and time-weighted per-instance losses before the final mean.
            ``None`` (the default) leaves the result bit-for-bit unchanged. The
            Zeva overlay drives this from ``r3_event_weighted``, which defaults to
            False, so the shipped recipes all take the ``None`` path.
    ...
    if sample_weights is not None:
        weights = sample_weights.to(
            device=per_instance_loss.device, dtype=per_instance_loss.dtype
        ).reshape(-1)
        if weights.shape[0] != per_instance_loss.shape[0]:
            raise ValueError(
                f"sample_weights must have one entry per instance: got {weights.shape[0]}, "
                f"expected {per_instance_loss.shape[0]}"
            )
        per_instance_loss = per_instance_loss * weights
        per_instance_weighted_loss = per_instance_weighted_loss * weights
    return (
