...
class GenerationDataClean:
    ...
    # Dense per-action-sample gate for the residual expert.  Values are in
    # [0,1] and aligned with x0_tokens_action; absent means exact base bypass.
    action_residual_gate: list[torch.Tensor] | None = None

    # Optional per-sample low-dimensional state used as causal prefix tokens.
    proprio_condition: list[torch.Tensor] | None = None
