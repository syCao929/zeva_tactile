...
class Cosmos3VFMNetworkConfig(PretrainedConfig):
    def __init__(
        ...
        behavior_stage2_config: dict | None = None,
        proprio_condition_config: dict | None = None,
        residual_expert_config: dict | None = None,
        **kwargs,
    ):
        ...
        self.behavior_stage2_config = behavior_stage2_config
        self.proprio_condition_config = proprio_condition_config
        self.residual_expert_config = residual_expert_config
...
class Cosmos3VFMNetwork(PreTrainedModel):
    ...
    def __init__(self, language_model, config: Cosmos3VFMNetworkConfig):
        ...
        if config.action_gen:
            ...
            residual_cfg = config.residual_expert_config or {}
            self.action_residual_head: nn.Linear | None = None
            if bool(residual_cfg.get("enabled", False)):
                self.action_residual_head = nn.Linear(self.hidden_size, self.action_dim, bias=True)
        else:
        ...
        if behavior_cfg is not None and behavior_cfg.get("enabled", False):
            ...
            prior_cfg = PolicyInjectionConfig(
                ...
                effect_history_length=int(behavior_cfg.get("effect_history_length", 4)),
                action_dim=int(behavior_cfg["action_dim"]),
                horizon=int(behavior_cfg["horizon"]),
                num_anchors=int(behavior_cfg["num_anchors"]),
            ...
            self.behavior_online_projector: nn.Linear | None = (
                nn.Linear(online_dim, self.hidden_size, bias=True) if online_enabled else None
            )
            self.behavior_prior_dropout_rate = float(behavior_cfg["prior_dropout_rate"])
            self.behavior_prior_inference_guidance_scale = float(
                behavior_cfg.get("prior_inference_guidance_scale", 0.5)
            )
        ...
        if proprio_cfg is not None and proprio_cfg.get("enabled", False):
            if not config.action_gen:
                raise ValueError("Proprio conditioning requires action_gen=True")
            self.proprio_input_dim = int(proprio_cfg["input_dim"])
            self.proprio_prefix_tokens = int(proprio_cfg.get("prefix_tokens", 1))
            if self.proprio_input_dim < 1 or self.proprio_prefix_tokens < 1:
                raise ValueError("Proprio input_dim and prefix_tokens must be positive")
            self.proprio_projector: nn.Linear | None = nn.Linear(
                self.proprio_input_dim,
                self.proprio_prefix_tokens * self.hidden_size,
                bias=True,
            )
        else:
            self.proprio_input_dim = 0
            self.proprio_prefix_tokens = 0
    ...
    def init_weights(self, buffer_device: torch.device | None):
        ...
        if self.config.action_gen:
            ...
            if self.action_residual_head is not None:
                # Zero initialization makes residual enabling an exact base
                # bypass before the first optimizer update.
                torch.nn.init.zeros_(self.action_residual_head.weight)
                torch.nn.init.zeros_(self.action_residual_head.bias)

        if (
            self.behavior_pbd is not None
            and self.behavior_adapter is not None
            and self.behavior_global_projector is not None
        ):
            self.behavior_pbd.init_weights()
    ...
    def _encode_behavior_prefix(
        self,
        packed_seq: PackedSequence,
        packed_sequence: torch.Tensor,
        target_dtype: torch.dtype,
    ) -> None:
        """Fill the learned global-behavior causal slots reserved by the packer."""
        if self.behavior_global_projector is None:
            if packed_seq.behavior_indexes.numel():
                raise ValueError("Packed behavior prefix slots require behavior_stage2.enabled=True")
            return
        if not packed_seq.behavior_indexes.numel():
            # Clean readout / vision-only calls intentionally contain no action
            # sample and therefore reserve no behavior prefix.
            return
        if not hasattr(packed_seq, "behavior_global"):
            raise ValueError("Stage-2 behavior prefix requires behavior_global on PackedSequence")
        global_features = packed_seq.behavior_global
        if global_features.ndim != 2 or global_features.shape[0] != packed_seq.behavior_indexes.numel():
            raise ValueError(
                "behavior_global must contain one vector per reserved behavior prefix, "
    ...
    def _encode_online_memory_prefix(
        ...
        """Write one causal prefix from same-task completed transitions."""
        indexes = packed_seq.online_memory_indexes
        if not indexes.numel():
            return
        if self.behavior_online_projector is None:
            raise ValueError("Online-memory prefix slots require behavior_stage2.online_memory_enabled=True")
        if not hasattr(packed_seq, "behavior_online_context"):
            raise ValueError("Online-memory prefix requires behavior_online_context on PackedSequence")
        context = packed_seq.behavior_online_context
        if context.ndim != 2 or context.shape[0] != indexes.numel():
            raise ValueError(
                "behavior_online_context must contain one vector per online prefix slot, "
                f"got {tuple(context.shape)} for {indexes.numel()} slots"
            )
        packed_sequence[indexes] = self.behavior_online_projector(context).to(target_dtype)

    def _encode_proprio_prefix(
        self,
        packed_seq: PackedSequence,
        packed_sequence: torch.Tensor,
        target_dtype: torch.dtype,
    ) -> None:
        """Project the robot state into reserved causal prefix slots."""
        if self.proprio_projector is None:
            if packed_seq.proprio_indexes.numel():
                raise ValueError("Packed proprio prefix slots require proprio_condition.enabled=True")
            return
        if not packed_seq.proprio_indexes.numel():
            return
        if not hasattr(packed_seq, "proprio_condition"):
            raise ValueError("Proprio prefix requires proprio_condition on PackedSequence")
        proprio = packed_seq.proprio_condition
        if proprio.ndim != 2 or proprio.shape[-1] != self.proprio_input_dim:
            raise ValueError(
                f"Expected proprio_condition [B,{self.proprio_input_dim}], got {tuple(proprio.shape)}"
            )
        prefix = self.proprio_projector(proprio).reshape(-1, self.hidden_size).to(target_dtype)
        if prefix.shape[0] != packed_seq.proprio_indexes.numel():
            raise ValueError("Proprio batch/prefix count does not match reserved slots")
        packed_sequence[packed_seq.proprio_indexes] = prefix
    ...
    def _encode_action(
        ...
        if self.behavior_pbd is not None and self.behavior_adapter is not None:
            ...
            residual = self.behavior_adapter(
                ...
                leading_condition_steps=self.behavior_prior_leading_condition_steps,
            )
            if self.training and self.behavior_prior_dropout_rate:
                keep = torch.rand((residual.shape[0], 1, 1), device=residual.device) >= self.behavior_prior_dropout_rate
                residual = residual * keep.to(residual.dtype)
            elif not self.training:
                residual = residual * self.behavior_prior_inference_guidance_scale
            packed_tokens_action = packed_tokens_action + residual.flatten(0, 1).to(packed_tokens_action.dtype)
            packed_seq.behavior_prior_mean = prior_mean
            packed_seq.behavior_prior_std = prior_std
    ...
    def _decode_action(
        ...
        if not has_noisy_action:
            ...
            # The frozen base output is always computed first.  R3's
            # zero-initialized residual expert is always on at inference: the
            # training-only event metadata must never become a phase/task
            # router.  The legacy explicit gate remains available only when
            # ``always_on`` is false, preserving the old experiment's exact
            # bypass semantics without confusing it with R3.
            residual_cfg = self.config.residual_expert_config or {}
            residual_always_on = bool(residual_cfg.get("always_on", False))
            if self.action_residual_head is not None and (residual_always_on or action.residual_gate is not None):
                residual_delta = self.action_residual_head(action_hidden_states)
                arm_limit = float(residual_cfg.get("arm_limit", 0.05))
                gripper_limit = float(residual_cfg.get("gripper_limit", 0.5))
                gripper_index = int(residual_cfg.get("gripper_index", 7 if self.action_dim == 8 else 9))
                if arm_limit < 0 or gripper_limit < 0:
                    raise ValueError("Residual arm/gripper limits must be non-negative")
                offset = 0
                bounded_parts: list[torch.Tensor] = []
                for sample_index, nfi in enumerate(action.noisy_frame_indexes):
                    count = int(nfi.numel())
                    delta_i = residual_delta[offset : offset + count]
                    if residual_always_on:
                        gate_i = torch.ones((count, 1), device=delta_i.device, dtype=delta_i.dtype)
                    else:
                        gate_i = action.residual_gate[sample_index].to(
                            device=delta_i.device, dtype=delta_i.dtype
                        )
                        gate_i = gate_i.index_select(0, nfi.to(device=gate_i.device)).reshape(-1, 1)
                    raw_dim = action.raw_action_dim[sample_index] if action.raw_action_dim is not None else None
                    delta_i = bounded_action_residual(
                        delta_i,
                        gate_i,
                        arm_limit=arm_limit,
                    ...
                    bounded_parts.append(delta_i)
                    offset += count
                if bounded_parts:
                    preds_action = preds_action + torch.cat(bounded_parts, dim=0)
    ...
    def forward(
        ...
        self._encode_behavior_prefix(packed_seq, packed_sequence, target_dtype)
        self._encode_online_memory_prefix(packed_seq, packed_sequence, target_dtype)
        self._encode_proprio_prefix(packed_seq, packed_sequence, target_dtype)
        ...
        packed_understanding_indexes = torch.cat(
            (
                packed_seq.behavior_indexes,
                packed_seq.online_memory_indexes,
                packed_seq.proprio_indexes,
        ...
        input_pack, attention_meta, natten_metadata_list = build_packed_sequence(
            ...
            packed_und_token_indexes=packed_understanding_indexes,
        ...
        if self.behavior_pbd is not None and hasattr(packed_seq, "behavior_prior_mean"):
            output_dict["behavior_prior_mean"] = packed_seq.behavior_prior_mean
            output_dict["behavior_prior_std"] = packed_seq.behavior_prior_std
