...
def pack_input_sequence(
    ...
    Args:
        ...
        num_behavior_prefix_tokens: Number of learned causal condition slots to
            reserve before text for each action-bearing sample. Zero preserves
            every existing Cosmos packing layout.
        ...
        if sequence_plan.has_text and not skip_text_tokens:
            ...
            text_sample_len = seq_builder.pack_text_tokens(
                ...
                num_behavior_prefix_tokens=(
                    num_behavior_prefix_tokens if sequence_plan.has_action else 0
                ),
                num_online_memory_prefix_tokens=(
                    num_online_memory_prefix_tokens if sequence_plan.has_action else 0
                ),
                num_proprio_prefix_tokens=(
                    num_proprio_prefix_tokens if sequence_plan.has_action else 0
                ),
            )
