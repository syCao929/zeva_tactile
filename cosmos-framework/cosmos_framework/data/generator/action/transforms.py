...
class ActionTransformPipeline:
    ...
    def __call__(
        ...
        residual_gate = data_dict.get("residual_gate")
        ...
        if history_action is not None and isinstance(action, torch.Tensor):
            ...
            if isinstance(residual_gate, torch.Tensor):
                residual_gate = torch.cat(
                    [torch.zeros((num_history_actions, 1), dtype=residual_gate.dtype), residual_gate], dim=0
                )
                data_dict["residual_gate"] = residual_gate
