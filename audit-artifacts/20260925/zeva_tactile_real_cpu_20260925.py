...
Run from zeva-work:
  CUDA_VISIBLE_DEVICES='' LD_LIBRARY_PATH='' PYTHONPATH=cosmos-framework \
    envs/zeva/bin/python audit-artifacts/20260925/zeva_tactile_real_cpu_20260925.py
...
def grad_probe() -> list[dict]:
    ...
    net.tactile_bit = TactileBIT(TactileBITConfig(memory_steps=30))
    ...
    import numpy as np
    parquet = REPO / "datasets/press_button_4_times_merged_filtered/xhand/PressButton4Times/lerobot_v21/lerobot/data/chunk-000/episode_000000.parquet"
    real_states = torch.tensor(pq.read_table(parquet, columns=["observation.state"]).to_pydict()["observation.state"], dtype=torch.float32)
    with np.load(REPO / "datasets/xhand_cte_features_v4/features_000000.npz") as z:
        cached = {k: torch.from_numpy(z[k]) for k in ("phase", "effect", "effect_valid")}
    ...
    for is_valid, gate_value in ((False, 0.), (False, 0.3), (True, 0.), (True, 0.3)):
        ...
        frame = 40 if is_valid else 0
        start = max(0, frame - 29)
        history = real_states[start:frame + 1]
        left_pad = 30 - len(history)
        batch["tactile_state"] = torch.cat((torch.zeros(left_pad, 1972), history))[None]
        batch["tactile_valid"] = torch.cat((torch.zeros(left_pad, dtype=torch.bool), torch.ones(len(history), dtype=torch.bool)))[None]
        batch["behavior_phase"] = cached["phase"][frame // 4].float()[None]
        batch["behavior_effect"] = cached["effect"][frame // 4].float()[None]
        batch["behavior_effect_valid"] = cached["effect_valid"][frame // 4][None]
        assert bool(batch["behavior_effect_valid"][0, -1]) == is_valid
        ...
        result = {
            ...
            "frame": frame,
            "tactile_shape": list(batch["tactile_state"].shape),
            "tactile_valid_frames": int(batch["tactile_valid"].sum()),
