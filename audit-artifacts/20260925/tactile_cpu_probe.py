...
Run from zeva-work:
  CUDA_VISIBLE_DEVICES='' LD_LIBRARY_PATH='' PYTHONPATH=cosmos-framework \
    envs/zeva/bin/python /tmp/zeva_tactile_audit_cpu_20260925.py
...
def grad_probe() -> list[dict]:
    ...
    net.tactile_phase_gate = nn.Parameter(torch.tensor([0.]))
    ...
    for is_valid in (False, True):
        ...
        mean, std = prior(packed.behavior_global, packed.behavior_phase, packed.behavior_effect, packed.behavior_effect_valid)
        ...
        assert all(v > 0 for v in vals) if is_valid else all(v == 0 for v in vals)
